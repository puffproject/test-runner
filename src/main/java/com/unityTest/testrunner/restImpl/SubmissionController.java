...
public class SubmissionController implements SubmissionApi {
	...
	@Override
	public ResponseEntity<SubmissionEvent> uploadSourceFiles(
			Principal principal,
			MultipartFile[] files,
			Integer assignmentId) throws IOException {
		...
		for (MultipartFile file : files) {
			...
			SubmissionFile submissionFile =
				new SubmissionFile(submission, file.getOriginalFilename(), file.getSize(), authorId, file.getBytes());
	...
	@Override
	public void downloadSourceFiles(Principal principal, HttpServletResponse response, Integer submissionId)
			throws IOException {
		...
		if (!authorId.equals(submission.getAuthorId()))
