...
public class SubmissionService {
	...
	public Submission getSubmissionById(int id) {
		...
		if (!opt.isPresent())
			throw new ElementNotFoundException(Submission.class, "id", String.valueOf(id));
		return opt.get();
	}

	public Submission getLatestSubmissionForAssignment(String authorId, int assignmentId) {
		Optional<Submission> opt =
			submissionRepository.findTopByAuthorIdAndAssignmentIdOrderByIdDesc(authorId, assignmentId);
		// Throw exception if a submission cannot be found
		if (!opt.isPresent())
			throw new ElementNotFoundException(Submission.class, "assignmentId", String.valueOf(assignmentId));
		return opt.get();
	}
}
