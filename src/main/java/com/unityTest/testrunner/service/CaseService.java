...
public class CaseService {
	...
	public Case createCase(Case testCase) {
		...
		if (testCase.getLanguage() != testCase.getSuite().getLanguage())
			throw new ProgrammingLanguageMismatch(testCase.getSuite().getLanguage(), testCase.getLanguage());
	...
	public Case createCase(TestCaseInfo caseInfo, String authorId) {
		...
		if (caseInfo.getLanguage() != suite.getLanguage())
			throw new ProgrammingLanguageMismatch(suite.getLanguage(), caseInfo.getLanguage());
		// Get test case code
		String code =
			codeService.buildTestCaseCode(caseInfo.getLanguage(), caseInfo.getFunctionName(), caseInfo.getBody());
		// Build case to save
		Case caseToCreate = new Case(
				suite, authorId, caseInfo.getFunctionName(), caseInfo.getDescription(), caseInfo.getLanguage(), code);
	 ...
	 * @param pLanguage Programming language of case to match
	 * @param authorId Author id of case to match
	 * @return Page view of test cases from repository matching passed arguments and formatted by the
	 *         pageable param
	 */
	...
	/**
	 * Get a list of test cases that match the given list of ids Optionally specify that all cases are
	...
	public List<Case> getCases(List<Integer> ids, Integer suiteId) throws ElementNotFoundException {
		...
		if (suiteId != null) {
			for (Case caze : cases) {
				if (caze.getSuite().getId() != suiteId)
					throw new ElementNotFoundException(
							Case.class, "id", String.valueOf(caze.getId()), "suite id", String.valueOf(suiteId));
			}
		}
		return cases;
	}
	...
	public Case getCaseById(int id) throws ElementNotFoundException {
		...
		if (!opt.isPresent())
			throw new ElementNotFoundException(Case.class, "id", String.valueOf(id));
		return opt.get();
	}
	...
	public Case updateCase(int id, TestCaseInfo caseInfo) {
		...
		String code = codeService
			.buildTestCaseCode(caseToUpdate.getLanguage(), caseToUpdate.getFunctionName(), caseInfo.getBody());
