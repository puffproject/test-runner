...
public interface SuiteApi extends BaseApi {
	 ...
	 */
	@ApiOperation(
		value = "Create a test suite",
		nickname = "createTestSuite",
		response = Suite.class,
	 ...
	 */
	@ApiOperation(
		value = "Upload the testing file for a test suite",
		nickname = "setTestSuiteFile",
		response = FileUploadEvent.class,
		produces = MediaType.APPLICATION_JSON_VALUE,
		consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	@PostMapping(
		value = "/{suiteId}/setFile",
		produces = MediaType.APPLICATION_JSON_VALUE,
		consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	ResponseEntity<FileUploadEvent> setTestSuiteFile(
			...
			@ApiParam(value = "File to set", required = true) @RequestParam("file") MultipartFile file)
			throws IOException;

	/**
	 * GET endpoint to retrieve test suites Filter by assignment id, name and programming language
	 * 
	 * @return Pageable view of test suites matching query criteria
	 */
	@ApiOperation(
		value = "Retrieve a pageable view of test suites",
		nickname = "getTestSuites",
		response = SuitePage.class,
		produces = MediaType.APPLICATION_JSON_VALUE)
	 ...
	 */
	@ApiOperation(
		value = "Get the contents of a test file for a test suite",
		nickname = "getTestSuiteFile",
		response = FileInfo.class,
		produces = MediaType.APPLICATION_JSON_VALUE)
	@GetMapping(value = "/{suiteId}/getFile", produces = MediaType.APPLICATION_JSON_VALUE)
	ResponseEntity<FileInfo> getTestSuiteFile(
			@ApiParam(value = "Test suite id", required = true) @PathVariable(value = "suiteId") Integer suiteId);
	 ...
	 */
	@ApiOperation(
		value = "Run test cases in a test suite",
		nickname = "runTestCases",
		response = TestResult.class,
		produces = MediaType.APPLICATION_JSON_VALUE)
	...
	ResponseEntity<ResponseBodyEmitter> runTestSuite(
			...
			@ApiParam(value = "Limit on number of cases run", defaultValue = "20")
			@RequestParam(value = "limit", required = false) Integer limit,
			@ApiParam(value = "Submission id")
