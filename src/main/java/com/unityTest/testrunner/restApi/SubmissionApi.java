...
public interface SubmissionApi extends BaseApi {
	...
	/**
	 * POST endpoint to upload source code files Note: Swagger-2 does not support uploading multiple
	 * files so will not work from the Swagger UI
	 * 
	 * @return Submission object as confirmation with file information
	 */
	@ApiOperation(
		value = "Upload source code files",
		nickname = "uploadSourceFiles",
		response = SubmissionEvent.class,
	...
	ResponseEntity<SubmissionEvent> uploadSourceFiles(
			...
			@ApiParam(name = "assignmentId", required = true) @RequestParam("assignmentId") Integer assignmentId)
			throws IOException;

	/**
	 * GET endpoint to download files uploaded by a submission as a zip
	 * 
	 * @param submissionId Id of submission to search for, downloading all source files uploaded by that
	 *        submission
	 */
	...
	void downloadSourceFiles(
			...
			@ApiParam(name = "submissionId", required = true)
			@RequestParam(value = "submissionId") Integer submissionId) throws IOException;
}
